...
- A PR must pass `PR policy`, both code/build jobs, migration tests, secret scan and dependency review. Netlify Deploy Preview is optional during the single-Staging phase.
...
4. Keep Netlify Deploy Preview checks optional while the unified Staging site is the only acceptance environment. Add them later only when concurrent PR previews are needed.
