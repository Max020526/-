...
- [ ] Deploy Preview uses Preview/Staging Supabase, never Production.
- [ ] Preview banner and `noindex,nofollow` are visible.
- [ ] Desktop/mobile and refreshed deep links work.

## Staging

- [ ] `develop` deployed to all three Staging sites.
...
- [ ] Complete `TEST-*` inbound → product completion → publish → storefront → order flow passed.
