...
export default function Login(){
  ...
  async function submit(e:FormEvent){
    ...
    if(error){setMessage("邮箱或密码不正确，请重新输入。");setLoading(false);return;}
    const {data:profile}=await client.from("profiles").select("role,is_active").eq("id",login.user.id).maybeSingle();
    if(profile?.is_active===false){await client.auth.signOut();setMessage("该账号已停用，请联系管理员。");setLoading(false);return;}
    setMessage("登录成功，正在返回系统…"); setLoading(false);
