...
type Product = {
  ...
  categories: { name: string } | null; product_images: Array<{ public_url: string; is_primary: boolean }>;
...
export default function Products() {
  ...
  const query = useCallback((client: any) => client.from("products")
    .select("id,style_no,name,name_zh,status,created_at,updated_at,cost_price,wholesale_price,retail_price,categories(name),product_images(public_url,is_primary),product_variants(id,inventory(quantity_on_hand,quantity_available))")
  ...
  return <main className="page"><PageHead eyebrow="PRODUCT CATALOG" title="商品管理" subtitle="搜索、筛选并完善入库自动创建的商品主档。" action={<Link className="button primary" href="/admin/products/new"><Plus size={15}/>新建商品</Link>}/><SetupBanner/>
    ...
    <section className="panel"><div className="panel-head"><div><h2>商品列表</h2><p>共 {filtered.length} 个商品</p></div><button className="button small" disabled title="CSV导出将在第12阶段启用"><Download size={14}/>导出</button></div>{visible.length ? <div className="table-wrap"><table className="data-table" style={{ minWidth: 1050 }}><thead><tr><th>主图</th><th>款号</th><th>商品名称</th><th>分类</th><th>颜色/SKU</th><th>总库存</th><th>成本价</th><th>批发价</th><th>零售价</th><th>状态</th><th>更新时间</th><th></th></tr></thead><tbody>{visible.map((product) => {
