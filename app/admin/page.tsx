...
export default function AdminDashboard() {
  const query = useCallback(async (client: any) => {
    ...
    const [products, published, pending, orders, inventory, inbound, newProducts, newVariants, ready] = await Promise.all([
    ...
    return { data: {
      ...
      ready: ready.count ?? 0,
      todayInbound: (inbound.data ?? []).reduce((sum: number, row: any) => sum + Number(row.total_quantity ?? 0), 0),
      todayInboundOrders: inbound.data?.length ?? 0,
  ...
  return <main className="page">
    ...
    <section className="stats-grid">
      <StatCard label="今日入库件数" value={data.todayInbound} note={`${data.todayInboundOrders} 张快速入库单`} icon={Warehouse}/>
    ...
    <section className="panel" style={{ marginBottom: 18 }}><div className="panel-head"><div><h2>管理快捷入口</h2><p>管理端专注商品、订单、库存控制和基础资料</p></div></div><div className="panel-body" style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><Link className="button primary" href="/admin/products/pending">待完善商品</Link><Link className="button" href="/admin/products/new">新建并上架</Link><Link className="button" href="/admin/orders">订单管理</Link><Link className="button" href="/admin/inventory">库存控制</Link><Link className="button" href="/settings/colors">颜色管理</Link></div></section>
