...
export function validateDeploymentEnvironment(env = process.env) {
  ...
  const siteUrl = (env.NEXT_PUBLIC_SITE_URL || env.DEPLOY_PRIME_URL || env.URL || "").trim();
  const internalApiUrl = (env.NEXT_PUBLIC_INTERNAL_API_URL || "").trim();
  ...
  if (!siteUrl.startsWith("https://") || /YOUR_/i.test(siteUrl)) issues.push("托管构建必须使用 HTTPS Site URL。");
  if (!internalApiUrl.startsWith("https://") || /YOUR_/i.test(internalApiUrl)) {
    issues.push("托管构建必须配置 HTTPS 的 NEXT_PUBLIC_INTERNAL_API_URL。");
  }
  if (!productionRef || /YOUR_/i.test(productionRef)) {
    issues.push("托管构建必须配置 PRODUCTION_SUPABASE_PROJECT_REF。");
...
function run() {
  ...
  if (issues.length) {
    console.error(`NEXORA 商城环境检查失败：\n- ${issues.join("\n- ")}`);
    process.exitCode = 1;
    return;
  }
  console.log("NEXORA 商城环境检查通过。");
}
