...
export function InventoryCenter({ mode }: { mode: Mode }) {
  ...
  const query = useCallback(async (client: any) => {
    const [inventory, movements] = await Promise.all([
      ...
      client.from("inventory_movements").select("id,movement_type,quantity_change,quantity_before,quantity_after,reference_no,notes,created_at,product_variants(sku,products(style_no,name)),warehouses(name),profiles(full_name)").order("created_at", { ascending: false }).limit(40),
    ]);
  ...
  return <main className="page">
    ...
    {selected && <section className="stock-adjuster">
